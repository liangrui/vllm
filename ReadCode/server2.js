...
function simpleMarkdownToHtml(mdContent) {
  ...
  // 关闭未关闭的表格标签
  if (insideTable) {
    html = html.replace(/(<table>[\s\S]*?)(?=(?:<h|$))/g, (match) => {
      return `${match}</table>`;
    });
  }
  ...
  // 9. 替换回 mermaid 代码块 - 用真实的 div 替换
  mermaidBlocks.forEach(({ id, code }) => {
    html = html.replace(`<!-- MERMAID_BLOCK_${id} -->`, `<div class="mermaid">${code}</div>`);
  });
...
<div class="container">${simpleMarkdownToHtml(content)}</div>
<script>
  mermaid.initialize({ 
    startOnLoad: true, 
    theme: 'dark', 
  ...
  document.addEventListener('DOMContentLoaded', function() {
    console.log('DOM loaded, checking for mermaid blocks');
    const mermaidBlocks = document.querySelectorAll('.mermaid');
    console.log('Found', mermaidBlocks.length, 'mermaid blocks');
    
    mermaidBlocks.forEach(block => {
      console.log('Mermaid block content:', block.textContent.substring(0, 100));
    });
  });
</script>
</body></html>`;
